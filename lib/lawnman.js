...
const parseArgs = allArgs => {
  return splitArgs(allArgs).map((args, i) => {
    const ext = args.shift()
    const script = args.join(' ')

    if (!ext) {
      throw new Error(`Missing extensions to watch (group ${i + 1})`)
    }

    if (!script) {
      throw new Error(`Missing script to run (group ${i + 1})`)
    }
